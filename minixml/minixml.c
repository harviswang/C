...
extern mxml_node_t *mxmlFindElement(mxml_node_t *node , mxml_node_t *top , const char *name , const char *attr ,
		const char *value , int descend);
extern mxml_node_t *mxmlLoadFile(mxml_node_t *top , djyfs_file *fp , mxml_type_t (*cb)(mxml_node_t *));
extern mxml_node_t *mxmlLoadString(mxml_node_t *top , const char *s , mxml_type_t (*cb)(mxml_node_t *));
...
extern char *mxmlSaveAllocString(mxml_node_t *node , int (*cb)(mxml_node_t * , int));
extern int mxmlSaveFile(mxml_node_t *node , djyfs_file *fp , int (*cb)(mxml_node_t * , int));
extern int mxmlSaveString(mxml_node_t *node , char *buffer , int bufsize , int (*cb)(mxml_node_t * , int));
               ...
               void *p) /* I - Pointer to file */
{
	if (ch < 128)
		...
		if (ch < 2048)
		{
			...
			if (putc(0xc0 | (ch >> 6), (djyfs_file *)p) < 0) //????
				return (-1);
			else
				return (putc(0x80 | (ch & 0x3f), (djyfs_file *)p));
		}
		else
			if (ch < 65536)
			{
				...
				if (putc(0xe0 | (ch >> 12), (djyfs_file *)p) < 0)
					return (-1);
				else
					if (putc(0x80 | ((ch >> 6) & 0x3f), (djyfs_file *)p) < 0)
						return (-1);
					else
						return (putc(0x80 | (ch & 0x3f), (djyfs_file *)p));
			}
			else
			{
				...
				if (putc(0xf0 | (ch >> 18), (djyfs_file *)p) < 0)
					return (-1);
				else
					if (putc(0x80 | ((ch >> 12) & 0x3f), (djyfs_file *)p) < 0)
						return (-1);
					else
						if (putc(0x80 | ((ch >> 6) & 0x3f), (djyfs_file *)p) < 0)
							return (-1);
